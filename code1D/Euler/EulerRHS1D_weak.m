...
% Extebd face data
rho_ext  =  apply_bc(rho(VtoE));
mmt_ext  =  apply_bc(mmt(VtoE));
Ener_ext =  apply_bc(Ener(VtoE));
...
fluxr = Euler_LF([rho_ext(2,2:K+1)' mmt_ext(2,2:K+1)' Ener_ext(2,2:K+1)'],...
                 [rho_ext(1,3:K+2)' mmt_ext(1,3:K+2)' Ener_ext(1,3:K+2)'],...
                 gas_gamma, gas_const)';
frhor = fluxr(1,:); fmmtr = fluxr(2,:); fenerr = fluxr(3,:);

fluxl = Euler_LF([rho_ext(2,1:K)'   mmt_ext(2,1:K)'   Ener_ext(2,1:K)'],...
