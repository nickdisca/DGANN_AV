...
% Extebd face data
depth_ext     =  Apply_BC1D(depth(VtoE),bc_cond(1,:));
discharge_ext =  Apply_BC1D(discharge(VtoE),bc_cond(2,:));

% Compute volume fluxes
fdep = discharge; fdis = 0.5*gravity*depth.^2 + discharge.^2./depth;

% Compute surface fluxes
fluxr = SWE_LF1D([depth_ext(2,2:K+1)' discharge_ext(2,2:K+1)'],...
               [depth_ext(1,3:K+2)' discharge_ext(1,3:K+2)'],...
               gravity)';
fdepr = fluxr(1,:); fdisr = fluxr(2,:);

fluxl = SWE_LF1D([depth_ext(2,1:K)' discharge_ext(2,1:K)'],...
