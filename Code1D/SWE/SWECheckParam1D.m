...
assert((strcmp(model,'SWE')),...
    'ERROR: ''model'' must be set to ''SWE'' only')

% gravity
assert(exist('gravity','var')==1,...
    'ERROR: ''gravity'' variable must be defined')
assert((isnumeric(gravity) & gravity > 0.0),...
...
% IC
assert(exist('depth_IC','var')==1,...
    'ERROR: ''depth_IC'' variable must be defined')

assert(exist('velocity_IC','var')==1,...
    'ERROR: ''velocity_IC'' variable must be defined')
...
Problem.depth_IC  = depth_IC;
Problem.vel_IC    = velocity_IC;
